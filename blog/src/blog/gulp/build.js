...
module.exports = function (options) {
    gulp.task('partials', ['markups'], function () {
        return gulp.src([
          ...
          .pipe($.angularTemplatecache('templateCacheHtml.js', {
            ...
            //---aligns src/app to app for gulp serve tasks
            .pipe($.replace(/src\/app\//g, 'app/'))
            //---------------------------------------------------
    ...
    gulp.task('other', function () {
        return gulp.src([
          options.src + '/**/*',
          '!' + options.src + '/**/*.{html,css,js,styl,ts,jade}'
        ])
          .pipe(gulp.dest(options.dist + '/'));
    });
    ...
    gulp.task('razor:clean', function (done) {
        $.del(options.views + '/Index.cshtml', done);
    });
    gulp.task('razor:move', function () {
        gulp.src(options.tmp + "/serve/base.html")
        .pipe($.rename("Index.cshtml"))
        .pipe(gulp.dest(options.views));
    });

    gulp.task('razor', ['razor:move'])
