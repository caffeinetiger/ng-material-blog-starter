module Common {
    ...
    var ezParameters = {
        ...
        websiteSuffix: 'com',
        currency: currency,
        appErrorPrefix: '[ZE Error] ', // configure the exceptionHandler decorator
        docTitle: 'ZoEazy: ',
        events: events,
        endPoints: {
            lookup: 'LookupAsync',
    ...
    export interface INgAuthSettings {
        existEndPoint: string;
        authServiceUri: string;
        endPoint: string;
        regEndPoint: string;
        reqConfEndPoint: string;
        postContent: string;
        postReqConf: string;
        header: string;
        clientId: string;
    ...
    export class AuthInterceptorService {
        ...
        static $inject = ['$location', '$q', 'LocalStorageService'];

        authKey: string = ngAuthSettings.authKey;
        userNotLogged: string = ngAuthSettings.events.userNotLogged;
        authData: { 'token': string } = null;
        $location: ng.ILocationService;
        $q: ng.IQService;
        localStorageService: ng.local.storage.ILocalStorageService;

        static AuthInterceptorServiceFactory($location: ng.ILocationService, $q: ng.IQService, localStorageService: ng.local.storage.ILocalStorageService) {
        ...
        constructor($location: ng.ILocationService,
            ...
            this.authData = this.localStorageService.get<{ 'token': string }>(this.authKey);
        }

        public request(requestConfig: any) {
            if (requestConfig.url.substring(8, 27) === 'maps.googleapis.com') return requestConfig;
